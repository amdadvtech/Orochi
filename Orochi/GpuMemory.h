...
class GpuMemory final
{
  public:
	...
	/// @brief Reset the memory space so that all bits inside are cleared to a value.
	/// @param value The value with which all bits are initialized.
	void reset( int value ) noexcept { OrochiUtils::memset( m_data, value, m_size * sizeof( T ) ); }
